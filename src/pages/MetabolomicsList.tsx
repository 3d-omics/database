...
const MetabolomicsList = () => {
  return (
    <div className='page_padding pt-7 min-h-[calc(100dvh-(var(--navbar-height)+var(--footer-height)))]'>
      <header className='main_header mb-4'>List of Metabolomics</header>
      ...
      <ul className='space-y-4'>
        {animalTrialExperimentData
          .filter((experiment) => (
            experiment.fields.ID === 'G' || experiment.fields.ID === 'H' || experiment.fields.ID === 'I' || experiment.fields.ID === 'J' || experiment.fields.ID === 'K'
          )).map((experiment) => (
            <li key={experiment.id}>
              <Link
                to={`/metabolomics/${encodeURIComponent(experiment.fields.Name)}`}
