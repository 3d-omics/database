...
const AnalysisSettings = ({ compareBetween, setCompareBetween, group1, setGroup1, group2, setGroup2, setExecuteCreatePlot, options }: {
        ...
        <section className='flex justify-between items-center gap-3 w-full max-sm:flex-col'>
          ...
          <div>
            ...
            <select
              ...
              id='group1'
            >
              {Object.keys(options[compareBetween]).map((option) => (
                <option key={option} value={option}>{options[compareBetween][option]}</option>
              ))
              }
            </select>
